...
class ThrottlerProtocol(Protocol):
    ...
    def is_ready(self, request: Request) -> bool:
        ...
        now, i.e. every time-based limit (delay, backoff) has elapsed *and* a concurrency slot is free in every scope.
...
class Throttler:
    ...
    def _cached_scope_ids(self, request: Request) -> list[ScopeID]:
        """Return the scope IDs of *request*, from the scopes returned by
        :meth:`get_resolved_scopes`."""
        scopes = self.get_resolved_scopes(request)
        # The readiness API asks for this on every queued scope set on every
        # dequeue, and default scoping yields a single scope; skipping the
        # general iteration there measures 2.3x faster.
        if isinstance(scopes, str):
            return [scopes]
        return list(iter_scopes(scopes))
    ...
    def _scopes_allow(self, request: Request, *, skip: ScopeID | None = None) -> bool:
        ...
        if self._request_delay_deadline(request, now) > now:
            return False
        for scope_id in self._cached_scope_ids(request):
            if scope_id == skip:
                continue
    ...
    def is_ready(self, request: Request) -> bool:
        ...
        if not self._unscheduled_waiters:
            return True
        return not self._unscheduled_claims(self._cached_scope_ids(request))
    ...
    def reserve(self, request: Request) -> None:
        ...
        self._record_reservation(
            request, self._resolve_scope_slots(self._cached_scope_ids(request))
        )
    ...
    def get_time_until_ready(self, request: Request) -> float | None:
        ...
        if not request.meta.get("dont_throttle"):
            for scope_id in self._cached_scope_ids(request):
                ...
                if manager is None:
                    continue
                wait = max(wait, manager.can_send(now=now))
